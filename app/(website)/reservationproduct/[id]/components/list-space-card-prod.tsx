  ...
  try {
    ...
    router.push(
      `/recap/accessoires/?type=reservation&name=${encodeURIComponent(
        ...
        date,
        isMonthlyTarif
      )}&quantity=${encodeURIComponent(data.quantity)}&productImage=${encodeURIComponent(
...
export default function ListeSpaceCardProd() {
  ...
  // Read query parameters to set initial product title
  useEffect(() => {
    const query = new URLSearchParams(window.location.search);
    const title = query.get("title");
    if (title) {
      const decodedTitle = decodeURIComponent(title);
      setProductTitle(decodedTitle);
      const foundProduct = AVANTAGES.find((item) => item.title.toLowerCase() === decodedTitle.toLowerCase());
      if (!foundProduct) {
        toast("Erreur", {
          description: "Produit non trouvé, veuillez sélectionner un produit valide",
        });
        setProductTitle(AVANTAGES[0].title);
      }
    } else {
      setProductTitle(AVANTAGES[0].title);
      toast("Avertissement", {
  ...
  // Select product based on productTitle
